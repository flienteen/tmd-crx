...
{
	if ( !isArray(id))
		$('#menuHere').before('<tr><td nowrap class="button" data-url="'+url+'"><table width="100%" border="0" cellpadding="0" cellspacing="0"><tr><td width="16"><img src="pic/'+img+'" width="16" height="16"></td> <td><span id="'+id+'"></span></td></tr></table></td></tr>');
	else{

		$('#menuHere').before('<tr><td nowrap class="button" data-url="'+url+'"><table width="100%" border="0" cellpadding="0" cellspacing="0"><tr><td width="16"><img id="pic_'+id[1]+'" src="pic/'+img+'" width="16" height="16"></td> <td><span id="'+id[0]+'"></span>(<span id="'+id[1]+'"></span>)</td></tr></table></td></tr>')
						;
	}
...
$(document).ready(function()
{
	...
	$('#popup-baloon tr td.button').bind('mouseup', function(e)
	{
		var url = $(this).data('url');

		if(url)
			return baseurl(url, e);
	}).bind('mousedown mouseup click dblclick contextmenu', function(){return false;});

	$('#popup-options td, #popup-empty td').unbind().bind('mouseup', function(e)
	{
		return baseurl('optionsPage',e)
	}).bind('mousedown mouseup click dblclick contextmenu', function(){return false;});
