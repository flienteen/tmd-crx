...
//		console.log(tmp);
		r = $(this).html().replace(new RegExp("(#\\d{1,}\\s[^\/><;:,\\s][^\/><;:,\\s]+)","img"), "<pfc>$1</pfc>").replace(new RegExp('("\\.{3}\\s.+\\s\\.{3}")', 'img'), "<pfci>$1</pfci>");
...
//	sURL = window.location.search;
	...
	if ( $('#forumShowTitle').length == 0 )
	{
		...
		$pageStyle.append("#forumShowTitle {-webkit-transition: opacity .3s ease-in-out; position:fixed; top:5px; left:12px; -webkit-border-radius:8px; border:1px solid #A79F72; background:#DCDCD2; padding:5px; opacity:0.5; cursor:default; -webkit-user-select:none; max-width:150px; text-align: center;} #forumShowTitle:hover {opacity: 1}");
	...
	if(cURL==='/forum.php' && fAction==='viewtopic')
	{
		...
		pTitle = $('title').text().replace(/.*:: (.{9,13}) -/,'');
